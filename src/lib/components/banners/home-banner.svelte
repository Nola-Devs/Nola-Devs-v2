...
<div
	class={twMerge("text-gray-900 max-h-64 md:max-h-80 bg-cover bg-no-repeat bg-white/10 p-8 rounded-lg shadow-md", bannerClass)}
>
	<div class="max-w-[672px]">
		<h2 class="text-2xl md:text-3xl font-bold mb-4 text-balance">{title}</h2>
		<p class="mb-8 leading-[24px] hidden sm:block sm:text-sm md:text-base dark:text-violet-300">{description}</p>
		<a
			href={linkHref}
			target="_blank"
