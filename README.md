...
If you don't want to install MongoDB natively, you can run it in a container.

**Install Docker** (if you don't already have it):

- **Mac / Windows**: install [Docker Desktop](https://www.docker.com/products/docker-desktop/) — pick the installer for your chip (Apple Silicon vs. Intel on Mac) and launch the app once installed so the Docker engine is running.
- **Linux**: follow the [Docker Engine install guide](https://docs.docker.com/engine/install/) for your distro (Ubuntu, Debian, Fedora, etc.), then run the [post-install steps](https://docs.docker.com/engine/install/linux-postinstall/) so you can use `docker` without `sudo`.
...
npm i
npm run start-local-db   # starts dockerized mongo + seeds it
npm run dev
```

`npm run start-local-db` brings up the `mongo:7` container defined in `docker-compose.yml` (exposed on `localhost:27017`, data persisted in a named volume), waits for it to accept connections, then runs `npm run seed`. Re-running it later is safe — it will reuse the existing container and reseed the data.

When you're done, `npm run stop-local-db` stops the container (data persists for next time). To fully tear it down: `docker compose down` (removes the container, keeps the volume) or `docker compose down -v` (wipes seeded data too).
